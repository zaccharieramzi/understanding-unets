from learning_wavelets.evaluate_tmp.focnet_eval import evaluate_focnet
from learning_wavelets.evaluate_tmp.results_to_csv import results_to_csv
from learning_wavelets.training.scripts.focnet_training import train_focnet

from generic_dask import train_eval_parameter_grid, eval_parameter_grid


def evaluate_focnet_parameters(
        n_filters=None,
        beta=None,
        noise_std_train=None,
        n_samples=None,
        exp_id='',
        **add_kwargs,
    ):
    if n_filters is None:
        n_filters = [128]
    if beta is None:
        beta = [0.2]
    if noise_std_train is None:
        noise_std_train = [(0, 55)]
    if n_samples is None:
        n_samples = [None]
    exp_id = f'focnet{exp_id}'
    param_grid = {
        'n_filters': n_filters,
        'beta': beta,
        'noise_std_train': noise_std_train,
        'n_samples': n_samples,
    }
    param_grid.update(**add_kwargs)
    metrics_names, results = train_eval_parameter_grid(
        exp_id,
        train_focnet,
        evaluate_focnet,
        parameter_grid=param_grid,
    )
    results_df = results_to_csv(
        metrics_names,
        results,
        output_path=f'{exp_id}.csv',
    )
    return results_df

def evaluate_focnet_runs(
        run_ids,
        n_filters=None,
        noise_std_train=None,
        n_samples=None,
        exp_id='',
        **add_kwargs,
    ):
    if n_filters is None:
        n_filters = [128]
    if beta is None:
        beta = [0.2]
    if noise_std_train is None:
        noise_std_train = [(0, 55)]
    if n_samples is None:
        n_samples = [None]
    exp_id = f'focnet{exp_id}'
    param_grid = {
        'n_filters': n_filters,
        'beta': beta,
        'noise_std_train': noise_std_train,
        'n_samples': n_samples,
    }
    param_grid.update(**add_kwargs)
    metrics_names, results = eval_parameter_grid(
        exp_id,
        evaluate_focnet,
        parameter_grid=param_grid,
        run_ids=run_ids,
    )
    results_df = results_to_csv(
        metrics_names,
        results,
        output_path=f'{exp_id}.csv',
    )
    return results_df
